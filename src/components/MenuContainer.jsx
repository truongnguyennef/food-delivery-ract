...
const MenuContainer = () => {
  ...
  return (
    <section className='w-full my-6' id='menu'>
        ...
        <p className='text-2xl font-semibold capitalize text-zinc-700 relative
            before:absolute before:rounded-lg before:content before:w-16 before:h-1 before:-bottom-2 
            before:left-0 before:bg-gradient-to-tr from-amber-400 to-amber-600 transition-all 
                      ...
                      >
                         ...
                         <div className={`w-10 h-10 rounded-full shadow-lg ${
                          filter === category.urlParamName ? 'bg-white' : 'bg-red-600'
                            ...
                            <IoFastFood 
                                className={`${
                                  filter === category.urlParamName ? 'text-zinc-700' : 'text-white'} group-hover:text-zinc-700 text-lg`}
                            />
                         </div>
                         <p className={`text-sm ${
                          filter === category.urlParamName ? 'text-white' : ' text-zinc-700'} group-hover:text-white`}>
                          {category.name}
                          </p>
                    </motion.div>
                ))}
